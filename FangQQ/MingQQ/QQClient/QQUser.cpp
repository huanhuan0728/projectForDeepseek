...
void CQQUser::offline(){
	CBuddyTeamInfo* pInfo = new CBuddyTeamInfo;
	pInfo->m_strName = _T("易道云学院");
	pInfo->m_nIndex = 0;
	CBuddyInfo* pUser = new CBuddyInfo;
	pUser->m_nQQNum = 6282371;
	pUser->m_nQQUin = 6282371;
	pUser->m_strNickName = _T("秦老师（Bingo）");
	pInfo->m_arrBuddyInfo.push_back(pUser);
	pUser = new CBuddyInfo;
	pUser->m_nQQNum = 1016555038;
	pUser->m_nQQUin = 1016555038;
	pUser->m_strNickName = _T("秦老师（Bingo）");
	pInfo->m_arrBuddyInfo.push_back(pUser);
	m_BuddyList.AddBuddyTeam(pInfo);
	pInfo = new CBuddyTeamInfo;
	pInfo->m_nIndex = 1;
	pInfo->m_strName = _T("AAA学员");
	pUser->m_nQQNum = 12345;
	pUser->m_nQQUin = 12345;
	pUser->m_strNickName = _T("VIP学员AAA");
	pInfo->m_arrBuddyInfo.push_back(pUser);
	m_BuddyList.AddBuddyTeam(pInfo);
}
